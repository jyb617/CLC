...
def data_load(dataset, has_v=False, has_a=True, has_t=True):
    ...
    return num_user, num_item, num_warm_item, train_data, val_data, val_warm_data, val_cold_data, test_data, test_warm_data, test_cold_data, a_feat, t_feat
...
class TrainingDataset(Dataset):
    ...
    def __getitem__(self, index):  # 构建正负样本对
        user, pos_item = self.train_data[index]

        # --- START FIX ---
        # Convert user and pos_item to integers
        user = int(user)
